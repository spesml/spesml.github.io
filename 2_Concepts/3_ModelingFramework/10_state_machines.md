...
- $\Delta \subseteq \Sigma \times (I\rightarrow M^\ast) \times (O\rightarrow M^\ast) \times \Sigma$ is a transition relation, relating a state and the inputs delivered to the machine to a follow-up state and output produced by the machine in this step. In case of a non-deterministic Mealy Machine, there may be several follow-up states and outputs related to a source state and input.
- The machine is required to be reactive, i.e., it holds that there exists a state $v \in \Sigma$ and an output $o: O \rightarrow M^\ast$ such that $(u,i,o,v) \in \Delta$ for all $u\in \Sigma$ and $i: I \rightarrow M^\ast$ such that 
...
The behavior of a run $r = t_0, t_1, ... \in \Delta^\infty$ where $t_j = (u_j, i_j, o_j, v_j)$ for all $j \in \mathcal{N}$ is defined as the tuple $(i,o)$ where $i = i_0, i_1,\ldots$ and $o=o_0,o_1,\ldots$.
...
$\Delta \subseteq \Sigma \times (I\rightarrow M^\ast \cup \{\checkmark\}) \times (O\rightarrow M^\ast \cup \{\checkmark\}) \times \Sigma$
...
- $\exists k \in M^\ast$ such that $u_j = k : \checkmark$
...
A patern in this sense is given by predicate $P:(C\rightarrow M^\ast)\rightarrow \mathbb{B}$. 

With this definition we can update the definition of the transition relation from above as:

$\Delta^p \subseteq \Sigma \times P_I \times (O\rightarrow M^\ast) \times \Sigma$, where $P_I$ denotes the set of patterns for channel set $I$. 
