...
1. Integration of technical components from various engineering disciplines. For instance, integration a mechatronic component (incl. mechanics, electronics, and software). Such a scenario can be supported by a representing technical component and their dedicated interfaces to other technical components.
...
For actually gaining an added value for systems engineering, an important aspect is to capture more than the structure of the hardware topology alone. The specification of relevant attributes of all technical entities is the key to enable the verification and analysis of the technical architecture.
For execution units, this might entail to capture the available memory and computational power.
For communication units, the usual technical aspects are bandwidth, real time capabilities and similar quality of service attributes.
